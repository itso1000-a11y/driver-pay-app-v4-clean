...
function getWeeklyRestSuggestionHelp(anchor: PreviousShiftAnchor | null): string {
  if (!anchor) return "";
  const fullStart = anchor.finishAbs + 45 * 60;
  const reducedStart = anchor.finishAbs + 24 * 60;
  const owed = 45 - 24;
  return `${t("weeklyRest45Option")}: ${formatShortDayTime(fullStart)} · ${t("weeklyRest24Option")}: ${formatShortDayTime(reducedStart)} (${owed}h ${t("owedLater")})`;
}
...
export default function App() {
  ...
  const weeklyRestSuggestionHelp = getWeeklyRestSuggestionHelp(previousShiftAnchor);
  const restContextHelp = getRestContextHelp(restBeforeMinutes);
