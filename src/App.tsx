...
function getWeekEndingLabel(days: DayRecord[]): string {
  ...
  return `Tax Week ${getTaxWeekNumber(date)} - ending ${formatDisplayDate(date)}`;
}
...
function MiniStat({ label, value, tone, compact }: { label: string; value: React.ReactNode; tone?: "danger"; compact?: boolean }) { const dangerStyle = tone === "danger" ? { border: "1px solid #ef4444", background: "linear-gradient(135deg,#ffffff 0%,#fee2e2 100%)", color: "#991b1b" } : { border: "1px solid #eef2f7", background: "#f8fafc", color: "#0f172a" }; return <div style={{ padding: compact ? 9 : 12, borderRadius: 14, ...dangerStyle }}><div style={{ fontSize: 12, color: tone === "danger" ? "#991b1b" : "#64748b", fontWeight: 700 }}>{label}</div><div style={{ fontSize: compact ? 16 : 17, fontWeight: 900, marginTop: 3 }}>{value}</div></div>; }
function RestCard({ value, colors }: { value: string; colors: { bg: string; border: string; text: string; label: string } }) { return <div style={{ padding: 9, borderRadius: 14, border: `1px solid ${colors.border}`, background: "rgba(255,255,255,0.65)", color: colors.text }}><div style={{ fontSize: 12, fontWeight: 800 }}>{colors.label}</div><div style={{ fontSize: 20, fontWeight: 900, marginTop: 2 }}>{value}</div></div>; }

type ToggleVariant = "danger" | "warning" | "success";
function ToggleRow({ label, value, onChange, right, variant }: { label: string; value: boolean; onChange: (checked: boolean) => void; right?: string; variant: ToggleVariant }) { const palettes: Record<ToggleVariant, { bg: string; border: string; text: string; shadow: string }> = { danger: { bg: "linear-gradient(135deg,#ffffff 0%,#fee2e2 100%)", border: "#ef4444", text: "#991b1b", shadow: "inset 0 3px 8px rgba(153,27,27,0.22)" }, warning: { bg: "linear-gradient(135deg,#ffffff 0%,#fed7aa 100%)", border: "#f97316", text: "#9a3412", shadow: "inset 0 3px 8px rgba(154,52,18,0.20)" }, success: { bg: "linear-gradient(135deg,#ffffff 0%,#dcfce7 100%)", border: "#22c55e", text: "#166534", shadow: "inset 0 3px 8px rgba(22,101,52,0.20)" } }; const p = palettes[variant]; const style: React.CSSProperties = value ? { ...buttonStyle, width: "100%", textAlign: "left", background: p.bg, border: `2px solid ${p.border}`, color: p.text, boxShadow: p.shadow, transform: "translateY(2px)", padding: "10px 10px" } : { ...buttonStyle, width: "100%", textAlign: "left", background: "#f8fafc", border: "1px solid #cbd5e1", color: "#475569", boxShadow: "0 2px 0 #cbd5e1", padding: "10px 10px" }; return <button type="button" style={style} onClick={() => onChange(!value)}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}><div><div style={{ fontSize: 15, fontWeight: 900 }}>{label}</div>{right && <div style={{ marginTop: 3, fontSize: 11, fontWeight: 800, opacity: 0.8 }}>{right}</div>}</div><div style={{ fontSize: 16, fontWeight: 950, letterSpacing: 0.6 }}>{value ? "✓" : ""}</div></div></button>; }
...
function BonusRow({ bonus, rate, onQtyChange, onDelete }: { bonus: BonusEntry; rate: string; onQtyChange: (value: string) => void; onDelete: () => void }) { return <div style={{ display: "grid", gridTemplateColumns: "1fr 86px auto", gap: 8, alignItems: "stretch" }}><div style={{ padding: "10px 12px", borderRadius: 14, border: "1px solid #cbd5e1", background: "#f8fafc" }}><div style={{ fontSize: 15, fontWeight: 900 }}>{bonus.type}</div><div style={{ marginTop: 3, fontSize: 12, color: "#64748b", fontWeight: 800 }}>{formatMoney(bonus.qty * parseDecimal(rate))}</div></div><input style={{ ...inputStyle, textAlign: "center", fontWeight: 900, padding: "10px 8px" }} inputMode="numeric" value={String(bonus.qty)} onChange={(e) => onQtyChange(e.target.value)} /><button style={{ ...buttonStyle, padding: "8px 10px", color: "#b91c1c" }} onClick={onDelete}>{t("delete")}</button></div>; }
