...
function getSuggestedStartHelp(suggested: SuggestedStarts): string {
  ...
  if (suggested.longPreviousShift) {
    ...
    parts.push(`${t("nineHourOption")}: ${minutesToTime(suggested.h9)}`);
  }
...
function getRestContextHelp(restMinutes: number | null): string {
  // Keep this card quiet: the card label + real rest time are enough.
  if (restMinutes == null) return "";
  if (restMinutes >= 9 * 60) return "";
  return t("violation");
}
...
export default function App() {
  ...
  function setCurrentDayType(type: DayType) {
    ...
    setDays((prev) => prev.map((day, index) => {
      ...
      if (type === "work") {
        const suggestedStart = weeklyRestPrimaryStart || getPrimarySuggestedStart(suggestedTimes);
        ...
        return { ...day, dayType: "work", start: day.start || suggestedStart, finishKm: autoGeneratedFinishKm ? "" : day.finishKm };
      }
  ...
  function saveAndGo() {
    ...
    const suggestedStart = weeklyRestPrimaryStart || dailyPrimarySuggestedStart;
    const rawStart = (weeklyRestCandidateActive && day.start && dailyPrimarySuggestedStart && day.start === dailyPrimarySuggestedStart && !dayHasDestructiveWorkData(day)) ? "" : day.start;
    const start = normalizeTime(rawStart || (day.finish && suggestedStart ? suggestedStart : ""));
    const finish = normalizeTime(day.finish || "");
    const s = parseTimeToMinutes(start);
    const f = parseTimeToMinutes(finish);
  ...
  const restBeforeColors = { ...statusPalette(effectiveRestStatus), label: getRestCardLabel(effectiveRestStatus, reducedCount) };
  ...
  const dailyStartIsManual = Boolean(currentDay.start && dailyPrimarySuggestedStart && currentDay.start !== dailyPrimarySuggestedStart);
  const weeklyStartIsManual = Boolean(currentDay.start && weeklyRestPrimaryStart && currentDay.start !== weeklyRestPrimaryStart);
  const weeklyRestSuggestionHelp = weeklyStartIsManual ? "" : weeklyRestSuggestionHelpRaw;
  const dailySuggestionHelp = (weeklyRestCandidateActive || dailyStartIsManual) ? "" : getSuggestedStartHelp(suggestedTimes);
  const startFieldHint = (!currentDay.start && !weeklyRestCandidateActive && dailyPrimarySuggestedStart && suggestedTimes.h11 != null) ? t("from11hRest") : "";
...
function TimeRow({ label, value, onChange, onBlur, placeholder = "00:00", hint = "" }: { label: string; value: string; onChange: (value: string) => void; onBlur: () => void; placeholder?: string; hint?: string }) { return <Field label={label}><div style={{ position: "relative" }}><input style={{ ...inputStyle, height: 58, fontSize: 24, fontWeight: 900, textAlign: "center", letterSpacing: 1, paddingLeft: 8, paddingRight: 8, paddingBottom: hint && !value ? 22 : 12 }} inputMode="numeric" value={value} onChange={(e) => onChange(e.target.value)} onBlur={onBlur} placeholder={placeholder} />{hint && !value && <div style={{ position: "absolute", left: 0, right: 0, bottom: 6, textAlign: "center", fontSize: 10, fontWeight: 800, color: "#64748b", pointerEvents: "none" }}>{hint}</div>}</div></Field>; }
