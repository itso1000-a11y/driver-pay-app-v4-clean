...
const pageStyle: React.CSSProperties = { minHeight: "100vh", background: "#f5f7fb", padding: 8, fontFamily: "Arial, sans-serif", color: "#0f172a" };
const shellStyle: React.CSSProperties = { maxWidth: 430, margin: "0 auto", background: "#ffffff", borderRadius: 20, border: "1px solid #e5e7eb", overflow: "hidden" };
const sectionStyle: React.CSSProperties = { padding: 11, borderTop: "1px solid #eef2f7" };
const inputStyle: React.CSSProperties = { width: "100%", padding: "9px 10px", borderRadius: 12, border: "1px solid #dbe3ee", fontSize: 14, outline: "none", boxSizing: "border-box", background: "#fff" };
const buttonStyle: React.CSSProperties = { borderRadius: 12, border: "1px solid #dbe3ee", padding: "9px 10px", fontSize: 14, fontWeight: 700, cursor: "pointer", background: "#fff", transition: "transform 0.08s ease, filter 0.08s ease, background 0.08s ease, opacity 0.08s ease, box-shadow 0.08s ease", WebkitTapHighlightColor: "transparent", userSelect: "none" };
...
function getRestFromPreviousShiftMinutes(anchor: PreviousShiftAnchor | null, current: DayRecord): number | null {
  ...
  let endAbs = getRestDisplayEndAbs(current);

  // First day of a new pay week can depend on the last completed shift from the
  // previous saved week. If the day is still in the future, the live "now" based
  // display returns null by design; for this bridge case we still need a factual
  // rest value so Start/rest suggestions do not disappear at week boundaries.
  if (endAbs == null && !current.start) {
    endAbs = getDayStartAbsMinutes(current);
  }
...
function getPreferredOpenDayIndex(days: DayRecord[]): number {
  ...
  if (weekSaturday === currentSaturday && todayIndex >= 0 && !isWeekClosed(weekSaturday)) {
    ...
    const isCurrentNonWorkingPlan = today.dayType === "holiday" || today.dayType === "off";
    ...
    // trap the user on a completed work day after Save & Next.
...
export default function App() {
  ...
  function placeCursorAtEnd(input: HTMLInputElement) {
    ...
    requestAnimationFrame(() => {
      try {
        ...
        // Some mobile browsers/input modes can reject selection changes; ignore safely.
      }
    });
  }
  ...
  function setCurrentDayType(type: DayType) {
    ...
    setDays((prev) => prev.map((day, index) => {
      ...
      if (type === "work") {
        ...
        return { ...day, dayType: "work", start: day.start || "", finishKm: autoGeneratedFinishKm ? "" : day.finishKm };
      }
      const carryKm = day.startKm || previousFinishKm || "";
      if (type === "holiday") return { ...day, dayType: "holiday", start: "", finish: "", holidayPay: day.holidayPay, startKm: carryKm, finishKm: carryKm, bonuses: [], nightOut: false, splitBreak: false };
      return { ...day, dayType: "off", start: "", finish: "", holidayPay: "", startKm: carryKm, finishKm: carryKm, bonuses: [], nightOut: false, splitBreak: false };
    }));
  }
  function saveAndGo() {
  ...
  if (!hasChosenLanguage) {
    return <div style={pageStyle}><div style={{ ...shellStyle, padding: 20 }}><div style={{ fontSize: 28, fontWeight: 900, marginBottom: 8 }}>{t("appTitle")}</div><div style={{ fontSize: 15, color: "#64748b", marginBottom: 16 }}>{t("chooseLanguage")}</div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}><button style={{ ...buttonStyle, background: "#0f172a", color: "white", borderColor: "#0f172a" }} onClick={() => setLanguage("en")}>English</button><button style={{ ...buttonStyle, background: "#0f172a", color: "white", borderColor: "#0f172a" }} onClick={() => setLanguage("bg")}>Български</button></div></div></div>;
  }

  return <div style={{ ...pageStyle, ...(archiveMode ? { background: "#cbd5e1" } : {}) }}><style>{`button{transition:transform .08s ease,filter .08s ease,background .08s ease,opacity .08s ease,box-shadow .08s ease;-webkit-tap-highlight-color:transparent;user-select:none}button:active:not(:disabled){transform:scale(.96);filter:brightness(.92)}button:disabled{opacity:.45;cursor:not-allowed}input,select{transition:border-color .12s ease,box-shadow .12s ease,background .12s ease}input:focus,select:focus{border-color:#94a3b8!important;box-shadow:0 0 0 3px rgba(148,163,184,.22)}`}</style><div style={{ ...shellStyle, position: "relative", ...(archiveMode ? { background: "#e2e8f0", borderColor: "#64748b", boxShadow: "0 0 0 4px rgba(100,116,139,.28)" } : {}) }}>{archiveMode && <div style={{ position: "absolute", inset: "150px 0 auto 0", textAlign: "center", pointerEvents: "none", zIndex: 0, fontSize: 58, fontWeight: 950, letterSpacing: 8, color: "rgba(71,85,105,.13)", transform: "rotate(-18deg)" }}>{t("archiveWatermark")}</div>}<div style={{ position: "relative", zIndex: 1 }}><Header currentDay={currentDay} weekEndingLabel={weekEndingLabel} onWeek={() => setShowWeekView(true)} onSettings={() => setShowSettings(true)} onInstall={installApp} canInstall={Boolean(installPrompt)} />{archiveMode && <div style={{ position: "sticky", top: 8, zIndex: 5, margin: 16, marginTop: 0, padding: 12, borderRadius: 14, background: "#cbd5e1", border: "2px solid #475569", color: "#1f2937", boxShadow: "0 10px 24px rgba(15,23,42,.16)" }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}><div><div style={{ fontSize: 14, fontWeight: 950, letterSpacing: .4 }}>{t("savedHistoricalWeek")}</div><div style={{ fontSize: 12, marginTop: 4 }}>{weekLocked ? t("historicalLocked") : t("editingArchive")}</div></div><div style={{ fontSize: 12, fontWeight: 950, padding: "6px 8px", borderRadius: 999, background: "#64748b", color: "white" }}>ARCHIVE</div></div><div style={{ display: "grid", gridTemplateColumns: weekLocked ? "1fr 1fr" : "1fr", gap: 8, marginTop: 10 }}><button style={{ ...buttonStyle, background: "#0f172a", color: "white", borderColor: "#0f172a" }} onClick={loadCurrentWeek}>{t("goToCurrentWeek")}</button>{weekLocked && <button style={{ ...buttonStyle, background: "#64748b", color: "white", borderColor: "#64748b" }} onClick={() => setHistoricalEditEnabled(true)}>{t("unlockEditing")}</button>}</div></div>}<div style={{ padding: 16, borderTop: "1px solid #eef2f7" }}><button type="button" style={{ ...buttonStyle, width: "100%", padding: 12, textAlign: "left", background: "#f8fafc", borderColor: "#eef2f7" }} onClick={() => setShowWeekPicker(true)}><div style={{ fontSize: 13, fontWeight: 800, color: "#334155", marginBottom: 6 }}>{t("weekEndingSaturday")}</div><div style={{ fontSize: 18, fontWeight: 900, color: "#0f172a" }}>{formatISODateDisplay(selectedSaturday)}</div></button><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7, marginTop: 10 }}><button style={buttonStyle} disabled={currentIndex === orderedIndices[0]} onClick={() => navigateLogical(-1)}>← {t("previous")}</button><button style={buttonStyle} disabled={currentIndex === orderedIndices[orderedIndices.length - 1]} onClick={() => navigateLogical(1)}>{t("next")} →</button></div></div><div style={sectionStyle}><SectionHeading title={t("dayType")} /><div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 6 }}><DayTypeButton label={t("workDay")} variant="work" active={currentDay.dayType === "work"} onClick={() => setCurrentDayType("work")} /><DayTypeButton label={t("holidayDay")} variant="holiday" active={currentDay.dayType === "holiday"} onClick={() => setCurrentDayType("holiday")} /><DayTypeButton label={t("offDay")} variant="off" active={currentDay.dayType === "off"} onClick={() => setCurrentDayType("off")} /></div></div>{currentDay.dayType === "work" && <div style={sectionStyle}><SectionHeading title={t("shift")} right={currentComputed.weekend ? t("weekend") : undefined} /><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, alignItems: "end" }}><TimeRow label={t("start")} value={displayStartValue} placeholder={dailyPrimarySuggestedStart || t("start")} hint={startFieldHint} onChange={(value) => updateTimeValue("start", value)} onBlur={() => normalizeTimeValue("start")} /><TimeRow label={t("finish")} value={currentDay.finish || ""} placeholder={t("finish")} onChange={(value) => updateTimeValue("finish", value)} onBlur={() => normalizeTimeValue("finish")} /></div>{dailySuggestionHelp && <HelperLine text={dailySuggestionHelp} />}{weeklyRestSuggestionHelp && <HelperLine text={weeklyRestSuggestionHelp} />}<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7, marginTop: 9 }}><MiniStat label={t("worked")} value={formatMinutes(currentComputed.workedMinutes)} tone={(currentComputed.workedMinutes ?? 0) > 15 * 60 ? "danger" : undefined} /><MiniStat label={t("ot")} value={currentComputed.overtimeMinutes > 0 ? formatMinutes(currentComputed.overtimeMinutes) : "—"} /></div>{shiftValidationMessage && <div style={{ marginTop: 10, fontSize: 13, fontWeight: 700, color: "#b91c1c" }}>{shiftValidationMessage}</div>}</div>}{currentDay.dayType === "work" && <div style={{ ...sectionStyle, background: activeRestColors.bg, paddingTop: 9, paddingBottom: 9 }}><SectionHeading title={t("restFromPreviousDay")} /><RestCard value={formatMinutes(restBeforeMinutes)} colors={activeRestColors} />{restContextHelp && <div style={{ marginTop: 6, fontSize: 12, color: activeRestColors.text, fontWeight: 800 }}>{restContextHelp}</div>}{currentIndex === orderedIndices[0] && !previousShiftAnchor && <div style={{ marginTop: 6, fontSize: 11, color: "#64748b" }}>{t("noPreviousDay")}</div>}</div>}<div style={sectionStyle}><SectionHeading title={t("kilometres")} right={startKmIsSuggested ? t("suggested") : undefined} /><div style={{ display: "grid", gridTemplateColumns: currentDay.dayType === "work" ? "1fr 1fr" : "1fr", gap: 10 }}><Field label={t("startKm")}><div style={{ position: "relative" }}><input style={{ ...inputStyle, fontSize: 17, fontWeight: 800, textAlign: "center", letterSpacing: 0, paddingLeft: 4, paddingRight: 4, minWidth: 0, color: startKmIsSuggested ? "#94a3b8" : "#0f172a", background: startKmIsSuggested ? "#f8fafc" : "#fff", paddingBottom: startKmIsSuggested ? 20 : 9 }} inputMode="numeric" value={displayStartKm} onFocus={(e) => startKmIsSuggested && placeCursorAtEnd(e.currentTarget)} onClick={(e) => startKmIsSuggested && placeCursorAtEnd(e.currentTarget)} onChange={(e) => updateKmValue("startKm", e.target.value)} placeholder="Start" />{startKmIsSuggested && <div style={{ position: "absolute", left: 0, right: 0, bottom: 5, textAlign: "center", fontSize: 9, fontWeight: 800, color: "#64748b", pointerEvents: "none" }}>{formatTemplate(t("fromFinishKm"), { source: startKmSuggestionSource === "last saved day" ? t("lastSavedDay") : t("lastWeek") })}</div>}</div></Field>{currentDay.dayType === "work" && <Field label={t("finishKm")}><input style={{ ...inputStyle, fontSize: 17, fontWeight: 800, textAlign: "center", letterSpacing: 0, paddingLeft: 4, paddingRight: 4, minWidth: 0 }} inputMode="numeric" value={currentDay.finishKm || ""} onChange={(e) => updateKmValue("finishKm", e.target.value)} placeholder="Finish" /></Field>}</div>{currentDay.dayType === "work" && <div style={{ marginTop: 8, padding: "8px 10px", borderRadius: 14, border: "1px solid #eef2f7", background: "#f8fafc" }}><div style={{ fontSize: 11, color: "#64748b", fontWeight: 700 }}>{t("kmRun")}</div><div style={{ fontSize: 15, fontWeight: 900, marginTop: 2 }}>{currentComputed.kmRun ?? "—"}</div></div>}</div>{currentDay.dayType === "holiday" && <div style={sectionStyle}><SectionHeading title={t("holidayPay")} right={t("taxed")} /><input style={inputStyle} type="text" inputMode="decimal" value={currentDay.holidayPay || ""} onChange={(e) => updateCurrentDay("holidayPay", e.target.value)} placeholder="0.00" /></div>}{currentDay.dayType === "work" && <div style={sectionStyle}><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}><ToggleRow label={t("nightOut")} variant="success" value={currentDay.nightOut} onChange={(checked) => updateCurrentDay("nightOut", checked)} /><ToggleRow label={t("splitBreak")} variant="warning" right={hasWeeklySplitBreak ? t("weekActive") : undefined} value={currentDay.splitBreak} onChange={(checked) => updateCurrentDay("splitBreak", checked)} /></div></div>}{currentDay.dayType === "work" && <div style={sectionStyle}><SectionHeading title={t("bonuses")} />{!showBonusForm && <button style={{ ...buttonStyle, background: "#0f172a", color: "white", borderColor: "#0f172a" }} onClick={() => setShowBonusForm(true)}>+ {t("addBonus")}</button>}{showBonusForm && <div style={{ display: "grid", gridTemplateColumns: "1fr 80px 88px", gap: 8 }}><select style={inputStyle} value={draftBonusType} onChange={(e) => setDraftBonusType(sanitizeBonusType(e.target.value))}>{activeBonusTypes.map((type) => <option key={type} value={type}>{type}</option>)}</select><input style={inputStyle} inputMode="numeric" value={draftBonusQty} onChange={(e) => setDraftBonusQty(digitsOnly(e.target.value))} /><button style={{ ...buttonStyle, background: "#0f172a", color: "white", borderColor: "#0f172a" }} onClick={addBonus}>{t("add")}</button></div>}<div style={{ display: "grid", gap: 7, marginTop: 9 }}>{currentDay.bonuses.map((bonus) => <BonusRow key={bonus.id} bonus={bonus} rate={getBonusRate(settings, bonus.type)} onDelete={() => removeBonus(bonus.id)} onQtyChange={(value) => updateBonusQty(bonus.id, value)} />)}</div></div>}<SummarySection currentComputed={currentComputed} dayType={currentDay.dayType} /><div style={{ ...sectionStyle, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}><button style={{ ...buttonStyle, background: "#0f172a", color: "white", borderColor: "#0f172a" }} onClick={saveAndGo}>{t("saveNext")}</button><button style={buttonStyle} onClick={() => setShowWeekView(true)}>{t("weekView")}</button></div>{showWeekPicker && <WeekPickerModal selectedSaturday={selectedSaturday} savedWeekIndicators={savedWeekIndicators} onSelectSaturday={(saturdayISO) => loadWeekBySaturday(saturdayISO, true)} onCurrentWeek={loadCurrentWeek} onClose={() => setShowWeekPicker(false)} />} {showSettings && <ModalErrorBoundary onClose={() => setShowSettings(false)}><SettingsModal settings={sanitizeSettings(settings)} setSettings={setSettings} days={days} setDays={setDays} archive={Array.isArray(archive) ? archive : []} setArchive={setArchive} payslipActualWeek={payslipActualWeek} setPayslipActualWeek={setPayslipActualWeek} setCurrentIndex={setCurrentIndex} setSelectedSaturday={setSelectedSaturday} setHistoricalEditEnabled={setHistoricalEditEnabled} language={language} setLanguage={setLanguage} onClose={() => setShowSettings(false)} /></ModalErrorBoundary>}{showWeekView && <WeekViewModal weekEndingLabel={weekEndingLabel} settings={settings} weekTotals={weekTotals} payslipActualWeek={payslipActualWeek} setPayslipActualWeek={setPayslipActualWeek} weekDifference={weekDifference} weekBonusSummary={weekBonusSummary} previewWeek={previewWeek} taxedWeek={taxedWeek} setCurrentIndex={setCurrentIndex} close={() => setShowWeekView(false)} endWeek={endWeek} goToCurrentWeek={() => { loadCurrentWeek(); setShowWeekView(false); }} />}</div></div></div>;
}

function Header({ currentDay, weekEndingLabel, onWeek, onSettings, onInstall, canInstall }: { currentDay: DayRecord; weekEndingLabel: string; onWeek: () => void; onSettings: () => void; onInstall: () => void; canInstall: boolean }) { const compactButtonStyle = { ...buttonStyle, padding: "7px 9px", fontSize: 12 }; return <div style={{ padding: "9px 12px 8px" }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 9 }}><div style={{ minWidth: 0 }}><div style={{ fontSize: 10, color: "#64748b", fontWeight: 700 }}>{t("appTitle")} · {t("currentDay")}</div><div style={{ fontSize: 22, fontWeight: 900, marginTop: 1, lineHeight: 1.0 }}>{dayNameLabel(currentDay.dayName)}</div><div style={{ fontSize: 18, color: "#0f172a", fontWeight: 900, marginTop: 1, lineHeight: 1.03 }}>{currentDay.dateLabel}</div><div style={{ fontSize: 12, color: "#334155", fontWeight: 800, marginTop: 4, lineHeight: 1.12 }}>{weekEndingLabel}</div></div><div style={{ display: "flex", gap: 5, flexWrap: "wrap", justifyContent: "flex-end" }}>{canInstall && <button style={compactButtonStyle} onClick={onInstall}>{t("install")}</button>}<button style={compactButtonStyle} onClick={onWeek}>{t("week")}</button><button style={compactButtonStyle} onClick={onSettings}>{t("settings")}</button></div></div></div>; }
function SectionHeading({ title, right }: { title: string; right?: string }) { return <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 6, marginBottom: 6 }}><div style={{ fontSize: 14, fontWeight: 900 }}>{title}</div>{right && <div style={{ fontSize: 11, fontWeight: 800, color: "#64748b" }}>{right}</div>}</div>; }
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label style={{ display: "grid", gap: 4 }}><div style={{ fontSize: 12, fontWeight: 800, color: "#334155" }}>{label}</div>{children}</label>; }
function TimeRow({ label, value, onChange, onBlur, placeholder = "Start", hint = "" }: { label: string; value: string; onChange: (value: string) => void; onBlur: () => void; placeholder?: string; hint?: string }) { return <Field label={label}><div style={{ position: "relative" }}><input style={{ ...inputStyle, height: 48, fontSize: 20, fontWeight: 900, textAlign: "center", letterSpacing: 0.5, paddingLeft: 7, paddingRight: 7, paddingBottom: hint && !value ? 19 : 9 }} inputMode="numeric" value={value} onChange={(e) => onChange(e.target.value)} onBlur={onBlur} placeholder={placeholder} />{hint && !value && <div style={{ position: "absolute", left: 0, right: 0, bottom: 5, textAlign: "center", fontSize: 9, fontWeight: 800, color: "#64748b", pointerEvents: "none" }}>{hint}</div>}</div></Field>; }
function HelperLine({ text }: { text: string }) { return <div style={{ marginTop: 7, fontSize: 12, fontWeight: 900, color: text.includes("unavailable") || text.includes("недостъпна") || text.includes("limit") ? "#b45309" : "#166534", lineHeight: 1.25 }}>{text}</div>; }
function MiniStat({ label, value, tone }: { label: string; value: React.ReactNode; tone?: "danger" }) { const danger = tone === "danger"; return <div style={{ padding: 9, borderRadius: 12, border: danger ? "1px solid #fecaca" : "1px solid #eef2f7", background: danger ? "#fef2f2" : "#f8fafc", color: danger ? "#991b1b" : undefined }}><div style={{ fontSize: 11, color: danger ? "#b91c1c" : "#64748b", fontWeight: 700 }}>{label}</div><div style={{ fontSize: 15, fontWeight: 900, marginTop: 2 }}>{value}</div></div>; }
function RestCard({ value, colors }: { value: string; colors: { bg: string; border: string; text: string; label: string } }) { return <div style={{ padding: 10, borderRadius: 12, border: `1px solid ${colors.border}`, background: "rgba(255,255,255,0.65)", color: colors.text }}>{colors.label && <div style={{ fontSize: 11, fontWeight: 800 }}>{colors.label}</div>}<div style={{ fontSize: 20, fontWeight: 900, marginTop: colors.label ? 3 : 0 }}>{value}</div></div>; }

type ToggleVariant = "danger" | "warning" | "success";
function ToggleRow({ label, value, onChange, right, variant }: { label: string; value: boolean; onChange: (checked: boolean) => void; right?: string; variant: ToggleVariant }) { const palettes: Record<ToggleVariant, { bg: string; border: string; text: string; shadow: string }> = { danger: { bg: "linear-gradient(135deg,#ffffff 0%,#fee2e2 100%)", border: "#ef4444", text: "#991b1b", shadow: "inset 0 3px 8px rgba(153,27,27,0.22)" }, warning: { bg: "linear-gradient(135deg,#ffffff 0%,#fed7aa 100%)", border: "#f97316", text: "#9a3412", shadow: "inset 0 3px 8px rgba(154,52,18,0.20)" }, success: { bg: "linear-gradient(135deg,#ffffff 0%,#dcfce7 100%)", border: "#22c55e", text: "#166534", shadow: "inset 0 3px 8px rgba(22,101,52,0.20)" } }; const p = palettes[variant]; const style: React.CSSProperties = value ? { ...buttonStyle, width: "100%", textAlign: "left", background: p.bg, border: `2px solid ${p.border}`, color: p.text, boxShadow: p.shadow, transform: "translateY(2px)", padding: "9px 10px" } : { ...buttonStyle, width: "100%", textAlign: "left", background: "#f8fafc", border: "1px solid #cbd5e1", color: "#475569", boxShadow: "0 2px 0 #cbd5e1", padding: "9px 10px" }; return <button type="button" style={style} onClick={() => onChange(!value)}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}><div><div style={{ fontSize: 14, fontWeight: 900 }}>{label}</div>{right && <div style={{ marginTop: 2, fontSize: 10, fontWeight: 800, opacity: 0.8 }}>{right}</div>}</div><div style={{ fontSize: 16, fontWeight: 950, letterSpacing: 0.6 }}>{value ? "✓" : ""}</div></div></button>; }

type DayButtonVariant = "work" | "holiday" | "off";
function DayTypeButton({ label, active, onClick, variant }: { label: string; active: boolean; onClick: () => void; variant: DayButtonVariant }) { const palettes: Record<DayButtonVariant, { bg: string; border: string; text: string }> = { work: { bg: "linear-gradient(135deg,#ffffff 0%,#dcfce7 100%)", border: "#22c55e", text: "#166534" }, holiday: { bg: "linear-gradient(135deg,#ffffff 0%,#fed7aa 100%)", border: "#f97316", text: "#9a3412" }, off: { bg: "linear-gradient(135deg,#ffffff 0%,#fee2e2 100%)", border: "#ef4444", text: "#991b1b" } }; const p = palettes[variant]; const style: React.CSSProperties = active ? { ...buttonStyle, width: "100%", textAlign: "center", background: p.bg, border: `2px solid ${p.border}`, color: p.text, boxShadow: "inset 0 4px 10px rgba(15,23,42,0.16)", transform: "translateY(2px)", padding: "7px 4px" } : { ...buttonStyle, width: "100%", textAlign: "center", background: "#f8fafc", border: "1px solid #cbd5e1", color: "#475569", boxShadow: "0 2px 0 #cbd5e1", padding: "7px 4px" }; const icons: Record<DayButtonVariant, string> = { work: "▣", holiday: "☀", off: "○" }; return <button type="button" style={style} onClick={onClick}><div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 3, minHeight: 20 }}><span aria-hidden="true" style={{ fontSize: 11, lineHeight: 1 }}>{icons[variant]}</span><span style={{ fontSize: 11, fontWeight: 950, lineHeight: 1.15 }}>{label}</span></div></button>; }

function Row({ label, value, strong }: { label: string; value: string; strong?: boolean }) { return <div style={{ display: "flex", justifyContent: "space-between", gap: 12, padding: "4px 0", fontSize: strong ? 15 : 13, fontWeight: strong ? 900 : 600 }}><div style={{ color: strong ? "#0f172a" : "#475569" }}>{label}</div><div>{value}</div></div>; }
function BonusRow({ bonus, rate, onDelete, onQtyChange }: { bonus: BonusEntry; rate: string; onDelete: () => void; onQtyChange: (value: string) => void }) { return <div style={{ display: "grid", gridTemplateColumns: "1fr 86px 44px", gap: 8, alignItems: "stretch" }}><div style={{ padding: "8px 10px", borderRadius: 12, border: "1px solid #dbe3ee", background: "#f8fafc" }}><div style={{ fontSize: 13, fontWeight: 900, color: "#0f172a" }}>{bonus.type}</div><div style={{ marginTop: 2, fontSize: 12, color: "#64748b", fontWeight: 700 }}>{formatMoney(bonus.qty * parseDecimal(rate))}</div></div><input style={{ ...inputStyle, padding: "8px 7px", textAlign: "center", fontWeight: 900 }} inputMode="numeric" value={String(bonus.qty)} onChange={(e) => onQtyChange(e.target.value)} /><button type="button" style={{ ...buttonStyle, padding: "7px 6px", color: "#b91c1c" }} onClick={onDelete}>×</button></div>; }
function SummarySection({ currentComputed, dayType }: { currentComputed: ComputedDay; dayType: DayType }) { const bonusText = currentComputed.bonuses.length ? currentComputed.bonuses.map((b) => `${b.type} x${b.qty}`).join(", ") : "—"; const isSoft = dayType === "holiday" || dayType === "off"; return <div style={{ ...sectionStyle, opacity: isSoft ? 0.68 : 1 }}><SectionHeading title={t("daySummary")} /><Row label={t("hours")} value={formatMinutes(currentComputed.workedMinutes)} /><Row label={t("overtime")} value={currentComputed.overtimeMinutes > 0 ? formatMinutes(currentComputed.overtimeMinutes) : "—"} /><Row label={t("km")} value={currentComputed.kmRun == null ? "—" : String(currentComputed.kmRun)} /><Row label={t("bonuses")} value={bonusText} /><Row label={t("nightOut")} value={currentComputed.nightOut ? t("yes") : t("no")} /></div>; }
...
function WeekViewModal(props: { weekEndingLabel: string; settings: SettingsState; weekTotals: WeekTotals; payslipActualWeek: string; setPayslipActualWeek: (value: string) => void; weekDifference: number; weekBonusSummary: Record<BonusType | "nightOuts", number>; previewWeek: ComputedDay[]; taxedWeek: ComputedDay[]; setCurrentIndex: (index: number) => void; close: () => void; endWeek: (type: WeekArchiveType, dayTypeOverrides?: Record<string, DayType>) => void; goToCurrentWeek: () => void }) {
  ...
  if (showCloseConfirm && isFastCloseWeek) {
    ...
    return <Overlay onClose={() => setShowCloseConfirm(false)}><ModalCard><ModalTitle>{t("futureWeekClose")}</ModalTitle><div style={{ fontSize: 13, color: "#475569", fontWeight: 700, marginBottom: 12 }}>{p.weekEndingLabel}</div><div style={{ padding: 12, borderRadius: 14, background: "#f8fafc", border: "1px solid #eef2f7", color: "#334155", fontSize: 13, fontWeight: 700 }}>{t("fastCloseHint")}</div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 7, marginTop: 9 }}><button style={closeTypeStyle} onClick={() => { setFastCloseMode("simple"); setAll("off"); }}>{t("allOff")}</button><button style={closeTypeStyle} onClick={() => { setFastCloseMode("simple"); setAll("holiday"); }}>{t("allHoliday")}</button></div><button style={{ ...buttonStyle, width: "100%", marginTop: 8, background: fastCloseMode === "choose" ? "#e2e8f0" : "#fff" }} onClick={() => setFastCloseMode(fastCloseMode === "choose" ? "simple" : "choose")}>{t("chooseDays")}</button>{fastCloseMode === "choose" && <div style={{ display: "grid", gap: 6, marginTop: 10 }}>{p.previewWeek.map((day) => <div key={day.id} style={{ display: "grid", gridTemplateColumns: "1fr 90px 90px", gap: 6, alignItems: "center" }}><div style={{ fontSize: 13, fontWeight: 900 }}>{dayNameLabel(day.dayName)}</div><button style={{ ...buttonStyle, padding: "8px 6px", background: futureTypes[day.id] === "off" ? "#dcfce7" : "#fff" }} onClick={() => setFutureTypes({ ...futureTypes, [day.id]: "off" })}>{t("off")}</button><button style={{ ...buttonStyle, padding: "8px 6px", background: futureTypes[day.id] === "holiday" ? "#fef3c7" : "#fff" }} onClick={() => setFutureTypes({ ...futureTypes, [day.id]: "holiday" })}>{t("holiday")}</button></div>)}</div>}<div style={{ display: "grid", gap: 7, marginTop: 9 }}><button style={{ ...buttonStyle, background: "#0f172a", color: "white", borderColor: "#0f172a", fontWeight: 900 }} onClick={() => closeAndExit("off", futureTypes)}>{t("closeFutureWeek")}</button><button style={buttonStyle} onClick={p.goToCurrentWeek}>{t("goToCurrentWeek")}</button><button style={buttonStyle} onClick={() => setShowCloseConfirm(false)}>{t("back")}</button></div></ModalCard></Overlay>;
  }
