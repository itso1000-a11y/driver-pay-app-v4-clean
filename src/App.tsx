...
type Lang = "en" | "bg";
const APP_VERSION = "v4.37.26-debug";
...
export default function App() {
  ...
  const diagnosticRows = [
    ...
    ["activeWeek", getSaturdayDay(days).dateISO],
    ["currentIndex", String(currentIndex)],
    ["currentDay", `${currentDay.dayLabel} ${currentDay.dateISO} / ${currentDay.dayType}`],
    ["current start/finish", `${currentDay.start || "-"} / ${currentDay.finish || "-"}`],
    ["previousDay", previousDay ? `${previousDay.dayLabel} ${previousDay.dateISO} ${previousDay.finish || "no finish"}` : "none"],
    ["previousShiftAnchor", previousShiftAnchor ? `${previousShiftAnchor.day.dayLabel} ${previousShiftAnchor.day.dateISO} finish ${previousShiftAnchor.day.finish} abs ${previousShiftAnchor.finishAbs}` : "none"],
    ["restBeforeMinutes", restBeforeMinutes == null ? "null" : `${restBeforeMinutes} (${formatMinutes(restBeforeMinutes)})`],
    ["suggested 11h", suggestedTimes.h11 == null ? "null" : absMinutesToLocalTime(suggestedTimes.h11)],
    ["suggested 9h", suggestedTimes.h9 == null ? "null" : absMinutesToLocalTime(suggestedTimes.h9)],
    ["lastFinishKmThisWeek", lastFinishKmThisWeek || "none"],
    ["previousWeekFinishKm", previousWeekFinishKm || "none"],
    ["previousFinishKm", previousFinishKm || "none"],
    ["displayStartKm", displayStartKm || "empty"],
    ["startKmIsSuggested", String(startKmIsSuggested)],
    ["source", startKmSuggestionSource || "none"],
    ["storage active", typeof window !== "undefined" ? (localStorage.getItem(ACTIVE_WEEK_STORAGE_KEY) || "empty") : "n/a"],
    ["week key exists", typeof window !== "undefined" ? String(Boolean(localStorage.getItem(getWeekStorageKey(getSaturdayDay(days).dateISO)))) : "n/a"],
    ["previous week key exists", typeof window !== "undefined" ? String(Boolean(localStorage.getItem(getWeekStorageKey(toISODate(addDays(fromISODate(getSaturdayDay(days).dateISO), -7)))))) : "n/a"],
  ];
