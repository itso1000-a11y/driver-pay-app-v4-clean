...
const pageStyle: React.CSSProperties = { minHeight: "100vh", background: "#f5f7fb", padding: 8, fontFamily: "Arial, sans-serif", color: "#0f172a", boxSizing: "border-box" };
const shellStyle: React.CSSProperties = { width: "100%", maxWidth: 430, margin: "0 auto", background: "#ffffff", borderRadius: 22, border: "1px solid #e5e7eb", overflow: "hidden", boxSizing: "border-box" };
const sectionStyle: React.CSSProperties = { padding: 14, borderTop: "1px solid #eef2f7" };
const inputStyle: React.CSSProperties = { width: "100%", padding: "11px 12px", borderRadius: 14, border: "1px solid #dbe3ee", fontSize: 16, outline: "none", boxSizing: "border-box", background: "#fff", minWidth: 0 };
const buttonStyle: React.CSSProperties = { borderRadius: 14, border: "1px solid #dbe3ee", padding: "11px 12px", fontSize: 14, fontWeight: 700, cursor: "pointer", background: "#fff", transition: "transform 0.08s ease, filter 0.08s ease, background 0.08s ease, opacity 0.08s ease, box-shadow 0.08s ease", WebkitTapHighlightColor: "transparent", userSelect: "none" };
...
function TimeRow({ label, value, onChange, onBlur, placeholder = "Start", hint = "" }: { label: string; value: string; onChange: (value: string) => void; onBlur: () => void; placeholder?: string; hint?: string }) { return <Field label={label}><div style={{ position: "relative" }}><input style={{ ...inputStyle, height: 56, fontSize: 22, fontWeight: 900, textAlign: "center", letterSpacing: 1, paddingLeft: 8, paddingRight: 8, paddingBottom: hint && !value ? 22 : 12 }} inputMode="numeric" value={value} onChange={(e) => onChange(e.target.value)} onBlur={onBlur} placeholder={placeholder} />{hint && !value && <div style={{ position: "absolute", left: 0, right: 0, bottom: 6, textAlign: "center", fontSize: 10, fontWeight: 800, color: "#64748b", pointerEvents: "none" }}>{hint}</div>}</div></Field>; }
...
function MiniStat({ label, value, tone }: { label: string; value: React.ReactNode; tone?: "danger" }) { const danger = tone === "danger"; return <div style={{ padding: 12, borderRadius: 14, border: danger ? "1px solid #fecaca" : "1px solid #eef2f7", background: danger ? "#fef2f2" : "#f8fafc", color: danger ? "#991b1b" : undefined }}><div style={{ fontSize: 12, color: danger ? "#b91c1c" : "#64748b", fontWeight: 700 }}>{label}</div><div style={{ fontSize: 16, fontWeight: 900, marginTop: 3, lineHeight: 1.15 }}>{value}</div></div>; }
...
function DayTypeButton({ label, active, onClick, variant }: { label: string; active: boolean; onClick: () => void; variant: DayButtonVariant }) { const palettes: Record<DayButtonVariant, { bg: string; border: string; text: string }> = { work: { bg: "linear-gradient(135deg,#ffffff 0%,#dcfce7 100%)", border: "#22c55e", text: "#166534" }, holiday: { bg: "linear-gradient(135deg,#ffffff 0%,#fed7aa 100%)", border: "#f97316", text: "#9a3412" }, off: { bg: "linear-gradient(135deg,#ffffff 0%,#fee2e2 100%)", border: "#ef4444", text: "#991b1b" } }; const p = palettes[variant]; const style: React.CSSProperties = active ? { ...buttonStyle, width: "100%", textAlign: "center", background: p.bg, border: `2px solid ${p.border}`, color: p.text, boxShadow: "inset 0 4px 10px rgba(15,23,42,0.16)", transform: "translateY(2px)", padding: "9px 4px", minWidth: 0 } : { ...buttonStyle, width: "100%", textAlign: "center", background: "#f8fafc", border: "1px solid #cbd5e1", color: "#475569", boxShadow: "0 2px 0 #cbd5e1", padding: "9px 4px", minWidth: 0 }; const icons: Record<DayButtonVariant, string> = { work: "▣", holiday: "☀", off: "○" }; return <button type="button" style={style} onClick={onClick}><div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 4, minHeight: 22, minWidth: 0 }}><span aria-hidden="true" style={{ fontSize: 12, lineHeight: 1, flex: "0 0 auto" }}>{icons[variant]}</span><span style={{ fontSize: 12, fontWeight: 950, lineHeight: 1.05, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "clip" }}>{label}</span></div></button>; }
