...
function Header({ currentDay, weekEndingLabel, onWeek, onSettings, onInstall, canInstall }: { currentDay: DayRecord; weekEndingLabel: string; onWeek: () => void; onSettings: () => void; onInstall: () => void; canInstall: boolean }) { const compactButtonStyle: React.CSSProperties = { ...buttonStyle, padding: "8px 10px", fontSize: 13, borderRadius: 12, whiteSpace: "nowrap" }; return <div style={{ padding: "10px 13px 9px" }}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}><div style={{ minWidth: 0, flex: "1 1 auto" }}><div style={{ fontSize: 11, color: "#64748b", fontWeight: 700, lineHeight: 1.1 }}>{t("appTitle")} · {t("currentDay")}</div><div style={{ fontSize: "clamp(24px, 8vw, 31px)", fontWeight: 900, marginTop: 2, lineHeight: .98 }}>{dayNameLabel(currentDay.dayName)}</div><div style={{ fontSize: "clamp(20px, 6vw, 25px)", color: "#0f172a", fontWeight: 900, marginTop: 2, lineHeight: 1 }}>{currentDay.dateLabel}</div><div style={{ fontSize: 13, color: "#334155", fontWeight: 800, marginTop: 5, lineHeight: 1.12, maxWidth: 190 }}>{weekEndingLabel}</div></div><div style={{ display: "grid", gap: 6, flex: "0 0 auto" }}>{canInstall && <button style={compactButtonStyle} onClick={onInstall}>{t("install")}</button>}<button style={compactButtonStyle} onClick={onWeek}>{t("week")}</button><button style={compactButtonStyle} onClick={onSettings}>{t("settings")}</button></div></div></div>; }
...
function DayTypeButton({ label, active, onClick, variant }: { label: string; active: boolean; onClick: () => void; variant: DayButtonVariant }) { const palettes: Record<DayButtonVariant, { bg: string; border: string; text: string }> = { work: { bg: "linear-gradient(135deg,#ffffff 0%,#dcfce7 100%)", border: "#22c55e", text: "#166534" }, holiday: { bg: "linear-gradient(135deg,#ffffff 0%,#fed7aa 100%)", border: "#f97316", text: "#9a3412" }, off: { bg: "linear-gradient(135deg,#ffffff 0%,#fee2e2 100%)", border: "#ef4444", text: "#991b1b" } }; const p = palettes[variant]; const shortLabel: Record<DayButtonVariant, string> = { work: t("workDay").startsWith("Work") ? "Work" : t("workDay"), holiday: t("holidayDay").startsWith("Holiday") ? "Holiday" : t("holidayDay"), off: t("offDay").startsWith("Day") ? "Off" : t("offDay") }; const style: React.CSSProperties = active ? { ...buttonStyle, width: "100%", textAlign: "center", background: p.bg, border: `2px solid ${p.border}`, color: p.text, boxShadow: "inset 0 4px 10px rgba(15,23,42,0.14)", transform: "translateY(1px)", padding: "9px 4px", overflow: "hidden" } : { ...buttonStyle, width: "100%", textAlign: "center", background: "#f8fafc", border: "1px solid #cbd5e1", color: "#475569", boxShadow: "0 2px 0 #cbd5e1", padding: "9px 4px", overflow: "hidden" }; const icons: Record<DayButtonVariant, string> = { work: "▣", holiday: "☀", off: "○" }; return <button type="button" style={style} onClick={onClick} title={label} aria-label={label}><div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 4, minHeight: 22, minWidth: 0, whiteSpace: "nowrap" }}><span aria-hidden="true" style={{ fontSize: 12, lineHeight: 1, flex: "0 0 auto" }}>{icons[variant]}</span><span style={{ fontSize: 12, fontWeight: 950, lineHeight: 1, overflow: "hidden", textOverflow: "clip", whiteSpace: "nowrap", minWidth: 0 }}>{shortLabel[variant]}</span></div></button>; }
