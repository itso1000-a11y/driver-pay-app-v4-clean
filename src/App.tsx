...
function WeekViewModal(props: { weekEndingLabel: string; weekTotals: WeekTotals; payslipActualWeek: string; setPayslipActualWeek: (value: string) => void; weekDifference: number; weekBonusSummary: Record<BonusType | "nightOuts", number>; previewWeek: ComputedDay[]; taxedWeek: ComputedDay[]; setCurrentIndex: (index: number) => void; close: () => void; endWeek: (type: WeekArchiveType, dayTypeOverrides?: Record<string, DayType>) => void; goToCurrentWeek: () => void }) {
  ...
  const weeklyBonusLabels = Object.entries(p.weekBonusSummary)
    .filter(([key, value]) => key !== "nightOuts" && Number(value) > 0)
    .map(([key, value]) => `${key} ${value}`);
  const weeklyBonusDisplay = weeklyBonusLabels.length ? weeklyBonusLabels.join(" • ") : (bonusCount ? String(bonusCount) : "—");
  const closeTypeStyle = { ...buttonStyle, fontWeight: 900 };
  const bonusTextForDay = (day: ComputedDay) => day.bonuses.length ? day.bonuses.map((b) => `${b.type} ${b.qty}`).join(", ") : "—";
  ...
  const usedExtras = [
    ...Object.entries(p.weekBonusSummary).filter(([key, value]) => key !== "nightOuts" && Number(value) > 0).map(([key, value]) => `${key} ${value}`),
