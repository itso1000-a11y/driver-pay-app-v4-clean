...
function formatISODateForDisplay(iso: string): string {
  return formatDisplayDate(fromISODate(iso));
}
...
export default function App() {
  ...
  function updateBonusQty(id: string, value: string) { const qty = Math.max(1, Number(digitsOnly(value) || "1") || 1); updateCurrentDay("bonuses", currentDay.bonuses.map((bonus) => bonus.id === id ? { ...bonus, qty } : bonus)); }
...
function RestCard({ value, colors }: { value: string; colors: { bg: string; border: string; text: string; label: string } }) { return <div style={{ padding: "9px 12px", borderRadius: 14, border: `1px solid ${colors.border}`, background: "rgba(255,255,255,0.65)", color: colors.text }}><div style={{ fontSize: 12, fontWeight: 800 }}>{colors.label}</div><div style={{ fontSize: 20, fontWeight: 900, marginTop: 2 }}>{value}</div></div>; }
...
function BonusRow({ bonus, rate, onDelete, onQtyChange }: { bonus: BonusEntry; rate: string; onDelete: () => void; onQtyChange: (value: string) => void }) { return <div style={{ display: "grid", gridTemplateColumns: "1fr 82px 58px", gap: 8, alignItems: "stretch" }}><div style={{ padding: "10px 12px", borderRadius: 14, border: "1px solid #cbd5e1", background: "#f8fafc", boxShadow: "0 2px 0 #cbd5e1" }}><div style={{ fontSize: 14, fontWeight: 900, color: "#334155" }}>{bonus.type}</div><div style={{ marginTop: 2, fontSize: 11, color: "#64748b", fontWeight: 800 }}>{formatMoney(bonus.qty * parseDecimal(rate))}</div></div><div style={{ padding: "8px 8px", borderRadius: 14, border: "1px solid #cbd5e1", background: "#fff", display: "grid", placeItems: "center" }}><input style={{ border: 0, outline: 0, background: "transparent", width: "100%", textAlign: "center", fontSize: 18, fontWeight: 900, color: "#0f172a" }} inputMode="numeric" value={String(bonus.qty)} onChange={(e) => onQtyChange(e.target.value)} aria-label={`${bonus.type} quantity`} /></div><button style={{ ...buttonStyle, padding: "8px 6px", color: "#b91c1c" }} onClick={onDelete}>×</button></div>; }
