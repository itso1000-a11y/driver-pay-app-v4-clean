...
const sectionStyle: React.CSSProperties = { padding: "13px 14px", borderTop: "1px solid #eef2f7" };
...
function getPrimarySuggestedStart(suggested: SuggestedStarts): string {
  if (suggested.h11 != null) return minutesToTime(suggested.h11);
  if (suggested.h9 != null && !suggested.h9Blocked) return minutesToTime(suggested.h9);
  return "";
}
...
function getPreferredOpenDayIndex(days: DayRecord[]): number {
  ...
  // Smart current-day priority: if the current/active week contains today,
...
export default function App() {
  ...
  function setCurrentDayType(type: DayType) {
    ...
    setDays((prev) => prev.map((day, index) => {
      ...
      if (type === "work") {
        const suggestedStart = getPrimarySuggestedStart(suggestedTimes);
        return { ...day, dayType: "work", start: day.start || suggestedStart };
      }
  ...
  const restBeforeColors = statusPalette(getEffectiveRestStatus(restBeforeMinutes, previousWorked, Boolean(previousDay?.splitBreak), reducedCount));
  ...
  function endWeek(type: WeekArchiveType, dayTypeOverrides?: Record<string, DayType>) {
    ...
    const markedDays = type === "worked" && !dayTypeOverrides ? days : days.map((d) => {
      if (!isEmptyForRemainingClose(d)) return d;
      const selectedType = dayTypeOverrides?.[d.id] || requestedType;
      return { ...d, dayType: selectedType, start: "", finish: "", holidayPay: selectedType === "holiday" ? d.holidayPay : "", bonuses: [], nightOut: false, splitBreak: false };
    });
...
function SectionHeading({ title, right }: { title: string; right?: string }) { return <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, marginBottom: 7 }}><div style={{ fontSize: 14, fontWeight: 900 }}>{title}</div>{right && <div style={{ fontSize: 12, fontWeight: 800, color: "#64748b" }}>{right}</div>}</div>; }
...
function TimeRow({ label, value, onChange, onBlur, placeholder = "00:00" }: { label: string; value: string; onChange: (value: string) => void; onBlur: () => void; placeholder?: string }) { return <Field label={label}><input style={{ ...inputStyle, height: 54, fontSize: 23, fontWeight: 900, textAlign: "center", letterSpacing: 1, paddingLeft: 8, paddingRight: 8 }} inputMode="numeric" value={value} onChange={(e) => onChange(e.target.value)} onBlur={onBlur} placeholder={placeholder} /></Field>; }
function HelperLine({ text }: { text: string }) { return <div style={{ marginTop: 8, fontSize: 13, fontWeight: 900, color: text.includes("unavailable") || text.includes("недостъпна") || text.includes("limit") ? "#b45309" : "#166534", display: "flex", alignItems: "center", gap: 6, lineHeight: 1.25 }}><span style={{ fontSize: 16 }}>!</span><span>{text}</span></div>; }
function MiniStat({ label, value, tone }: { label: string; value: React.ReactNode; tone?: "danger" }) { const danger = tone === "danger"; return <div style={{ padding: 10, borderRadius: 14, border: danger ? "1px solid #fecaca" : "1px solid #eef2f7", background: danger ? "#fef2f2" : "#f8fafc", color: danger ? "#991b1b" : undefined }}><div style={{ fontSize: 12, color: danger ? "#b91c1c" : "#64748b", fontWeight: 700 }}>{label}</div><div style={{ fontSize: 17, fontWeight: 900, marginTop: 3 }}>{value}</div></div>; }
function RestCard({ value, colors }: { value: string; colors: { bg: string; border: string; text: string; label: string } }) { return <div style={{ padding: 10, borderRadius: 14, border: `1px solid ${colors.border}`, background: "rgba(255,255,255,0.65)", color: colors.text }}><div style={{ fontSize: 12, fontWeight: 800 }}>{colors.label}</div><div style={{ fontSize: 21, fontWeight: 900, marginTop: 3 }}>{value}</div></div>; }
...
function DayTypeButton({ label, active, onClick, variant }: { label: string; active: boolean; onClick: () => void; variant: DayButtonVariant }) { const palettes: Record<DayButtonVariant, { bg: string; border: string; text: string }> = { work: { bg: "linear-gradient(135deg,#ffffff 0%,#dcfce7 100%)", border: "#22c55e", text: "#166534" }, holiday: { bg: "linear-gradient(135deg,#ffffff 0%,#fed7aa 100%)", border: "#f97316", text: "#9a3412" }, off: { bg: "linear-gradient(135deg,#ffffff 0%,#fee2e2 100%)", border: "#ef4444", text: "#991b1b" } }; const p = palettes[variant]; const style: React.CSSProperties = active ? { ...buttonStyle, width: "100%", textAlign: "center", background: p.bg, border: `2px solid ${p.border}`, color: p.text, boxShadow: "inset 0 4px 10px rgba(15,23,42,0.16)", transform: "translateY(2px)", padding: "8px 5px" } : { ...buttonStyle, width: "100%", textAlign: "center", background: "#f8fafc", border: "1px solid #cbd5e1", color: "#475569", boxShadow: "0 2px 0 #cbd5e1", padding: "8px 5px" }; const icons: Record<DayButtonVariant, string> = { work: "▣", holiday: "☀", off: "○" }; return <button type="button" style={style} onClick={onClick}><div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 5, minHeight: 22 }}><span aria-hidden="true" style={{ fontSize: 13, lineHeight: 1 }}>{icons[variant]}</span><span style={{ fontSize: 13, fontWeight: 950, lineHeight: 1.15 }}>{label}</span></div></button>; }
...
function BonusRow({ bonus, rate, onDelete, onQtyChange }: { bonus: BonusEntry; rate: string; onDelete: () => void; onQtyChange: (value: string) => void }) { return <div style={{ display: "grid", gridTemplateColumns: "1fr 78px 40px", gap: 7, alignItems: "stretch" }}><div style={{ padding: "9px 10px", borderRadius: 14, border: "1px solid #dbe3ee", background: "#f8fafc" }}><div style={{ fontSize: 13, fontWeight: 900, color: "#0f172a" }}>{bonus.type}</div><div style={{ marginTop: 2, fontSize: 12, color: "#64748b", fontWeight: 700 }}>{formatMoney(bonus.qty * parseDecimal(rate))}</div></div><input style={{ ...inputStyle, padding: "10px 8px", textAlign: "center", fontWeight: 900 }} inputMode="numeric" value={String(bonus.qty)} onChange={(e) => onQtyChange(e.target.value)} /><button type="button" style={{ ...buttonStyle, padding: "8px 6px", color: "#b91c1c" }} onClick={onDelete}>×</button></div>; }
...
function SettingsInput({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return <Field label={label}><input style={{ ...inputStyle, marginBottom: 8 }} type="text" inputMode="decimal" value={value || ""} onChange={(e) => onChange(e.target.value)} /></Field>;
}
