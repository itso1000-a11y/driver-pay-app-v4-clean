...
function getWeeklyRestContextHelp(restMinutes: number | null, requiredMinutes: number): string {
  ...
  if (requiredMinutes > 45 * 60) {
    ...
    return restMinutes >= requiredMinutes ? "" : `${missing}h ${t("toFullWeeklyRest")}`;
  }
...
export default function App() {
  ...
  const currentDayStartAbs = getDayStartAbsMinutes(currentDay);
  const weeklyRestAgeMinutes = weeklyRestCandidate ? currentDayStartAbs - weeklyRestCandidate.finishAbs : null;
  const weeklyRestWithinSuggestionWindow = weeklyRestAgeMinutes == null || weeklyRestAgeMinutes <= 74 * 60;
  const weeklyRestBaseActive = Boolean(weeklyRestCandidate && selectedSaturday > weeklyRestCandidate.closingSaturdayISO && currentDay.dayType === "work" && !hasCompletedWorkBeforeCurrentInThisWeek && weeklyRestWithinSuggestionWindow);
  ...
  const startSuggestionLabel = weeklyRestPrimaryStart ? t("startFromWeekly") : (dailyPrimarySuggestedStart ? (suggestedTimes.h11 != null && dailyPrimarySuggestedStart === minutesToTime(suggestedTimes.h11) ? t("startFrom11h") : t("startFromReduced")) : "");
...
function TimeRow({ label, value, onChange, onBlur, placeholder = "00:00", suggestionLabel = "" }: { label: string; value: string; onChange: (value: string) => void; onBlur: () => void; placeholder?: string; suggestionLabel?: string }) { const isSuggestion = !value && placeholder !== "00:00" && Boolean(suggestionLabel); return <Field label={label}><div style={{ position: "relative" }}><input style={{ ...inputStyle, height: 58, fontSize: 24, fontWeight: 900, textAlign: "center", letterSpacing: 1, paddingLeft: 8, paddingRight: 8, paddingBottom: isSuggestion ? 22 : undefined, color: isSuggestion ? "#94a3b8" : "#0f172a", background: isSuggestion ? "#f8fafc" : "#fff" }} inputMode="numeric" value={value} onChange={(e) => onChange(e.target.value)} onBlur={onBlur} placeholder={placeholder} />{isSuggestion && <div style={{ position: "absolute", left: 0, right: 0, bottom: 6, textAlign: "center", fontSize: 10, fontWeight: 800, color: "#64748b", pointerEvents: "none" }}>{suggestionLabel}</div>}</div></Field>; }
